...
public class JointOrientation : MonoBehaviour
{
    ...
    // Set by making the fingers spread pose or dpressing "r".
    ...
    void Update ()
    {
		...
		//float x = (Screen.width-Input.mousePosition.x - 0) * (7.3f) / Screen.width-3;//-offset.x;//subtract a1, multiply range2/range1, add a2
		//float y = (Input.mousePosition.y - 0) * (2.8f) / Screen.height +.5f;//-offset.y;
        ...
        if (updateReference) {
            ...
            _antiYaw = Quaternion.FromToRotation (
                new Vector3 (myo.transform.forward.x, 0, myo.transform.right.z),
                new Vector3 (0, 0, 1)
            );
    ...
    float rollFromZero (Vector3 zeroRoll, Vector3 forward, Vector3 up)
    {
        ...
        float cosine = Vector3.Dot (forward, zeroRoll);
